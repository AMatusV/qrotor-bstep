...
U2 = q*r*(Iz - Iy)*Ix + Ix*(-dxdt(7)*(kn1 + ko1) - (1 + kn1*ko1)*(phi - phit) + (dxdt(9)*(kn3 + ko1) + (1 + kn3*ko1)*(psi - psit))*sin(theta));
U3 = p*r*(Ix - Iz)*Iy + Iy*((-(dxdt(8)*(kn2 + ko2) + (1 + kn2*ko2)*(theta - thetat)))*cos(phi) - (dxdt(9)*(kn3 + ko2) + (1 + kn3*ko2)*(psi - psit))*cos(theta)*sin(phi));
U4 = p*q*(Iy - Ix)*Iz + Iz*((-(dxdt(9)*(kn3 + ko3) + (1 + kn3*ko3)*(psi - psit)))*cos(phi)*cos(theta) + (dxdt(8)*(kn2 + ko3) + (1 + kn2*ko3)*(theta - thetat))*sin(phi));
