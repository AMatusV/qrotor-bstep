...
%  Hans Bruun Nielsen,  IMM, DTU.  99.12.07
   ...
   alpha = 0;   fn = f;   gn = g; 
   if  length(lpar) > 5
     [n,h, b,fib,g,spar] = check(x,fn,gn,h,lpar,[nargin nargout], fun);
     neval = 1;
   else
   ...
   case  1    % Exact line search
     ...
     while  more
       if  fib < fi0    % New lower bound
         ...
         if  (neval < spar(4)) & (b < spar(1))
           ...
           [fib  g] = feval(fun, x+b*h);   neval = neval+1;
           dfib = dot(g,h); 
     ...
     % Refine interval
       ...
       while  (neval < spar(4)) & (abs(xfd(3,3)) > slopethr) ...
         ...
         c = interpolate(xfd,n);
         [fic g] = feval(fun, x+c*h);   neval = neval+1;
         xfd(3,:) = [c  fic  dot(g,h)];
...
function  [n,h, b, f, g, parms] = check(x,f,g, h, lpar, nargs, fun)
%  Check function call
