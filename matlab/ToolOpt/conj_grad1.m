function  [X, info, perf] = conj_grad1(fun, x0, opts)
...
%
%  MODIFICACION POR GRG 5/MAYO/05:
%            cambiamos fun(x,par) por fun(x)
%
%            function  [f, g] = fun(x)
...
%  Hans Bruun Nielsen,  IMM, DTU.  99.08.05-09 / 08.23 / 12.07
   ...
   %  Check call 
   [x  n  f  g  opts] = check(fun,x0,opts);
   ...
   while  ~found
     ...
     if  k > 2,  sc = .9*alpha1*nhpr/nh;
     else    % First step.  Try not to move too far
       sc = opts(3)/32/nh;
     end
     %  Line search
     lspar = [32  opts([2 7:8])  min(kmax-neval, opts(9))];
     [al  f  g  dval] = linesearch(fun,x,f,g, sc*h, lspar);
     alpha1 = sc*al;   ng = norm(g, inf);   n2g = norm(g);  gg = n2g^2;
     %  Update  gamma  and  x
     if  opts(1) == 1,  gam = gg/ggpr;
     else,              gam = dot(g-gpr,g)/ggpr; end
     x = x + alpha1*h;   neval = neval + dval;
     if  Trace
           X(:,k) = x(:);   perf(1:2,k) = [f; ng];
           perf(3:5,k-1) = [alpha1; dot(h,g); dval]; end
     %  Check stopping criteria
     if      ng <= opts(4),                            found = 1;
     elseif  alpha1*nh <= opts(5)*(opts(5) + norm(x)),  found = 2;
   ...
   info = [f  ng  alpha1*nh  k-1  neval  found];

% ==========  auxiliary function  =================================

function  [x,n, f,g, opts] = check(fun,x0,opts0)
%  Check function call
   sx = size(x0);   n = max(sx);
   %if  (min(sx) > 1)
   %    error('x0  should be a vector'), end
   x = x0;   [f g] = feval(fun,x);
   sf = size(f);   sg = size(g);
